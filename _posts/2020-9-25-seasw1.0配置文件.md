...
```shell
[root@localhost app]# cat seasw_rdy.cli 
;定义内存池
mempool MEMPOOL0 buffer 2304 pool 1024K cache 256 cpu 0
mempool MEMPOOL1 buffer 2304 pool 512K cache 256 cpu 0

;定义控制器，其中192.168.9.225为控制器ip地址，dpid 203为交换机dpid，在网络中唯一
protocol PROT0 mempool MEMPOOL1 pof 192.168.9.225 6643 dpid 203

;4个FE核，每个FE核负责一个link网卡的数据包处理。每个FE的处理定义复用pipeline P0
;绑定的网卡，因为有4个FE核分别负责4个网卡，所以每张网卡有一个收包队列（in one），四个发包队列
link LINK0 dev 0000:3b:00.0 rxq 1 1024 MEMPOOL0 txq 4 1024 promiscuous on
link LINK1 dev 0000:3b:00.1 rxq 1 1024 MEMPOOL0 txq 4 1024 promiscuous on
link LINK2 dev 0000:3b:00.2 rxq 1 1024 MEMPOOL0 txq 4 1024 promiscuous on
link LINK3 dev 0000:3b:00.3 rxq 1 1024 MEMPOOL0 txq 4 1024 promiscuous on
...
pipeline P0 port in bsz 32 link LINK1 rxq 0
pipeline P0 port in bsz 32 link LINK2 rxq 0
pipeline P0 port in bsz 32 link LINK3 rxq 0
 
pipeline P0 port out bsz 1 link LINK0 txq 0
pipeline P0 port out bsz 1 link LINK0 txq 1
pipeline P0 port out bsz 1 link LINK0 txq 2
pipeline P0 port out bsz 1 link LINK0 txq 3

pipeline P0 port out bsz 1 link LINK1 txq 0
pipeline P0 port out bsz 1 link LINK1 txq 1
pipeline P0 port out bsz 1 link LINK1 txq 2
pipeline P0 port out bsz 1 link LINK1 txq 3

pipeline P0 port out bsz 1 link LINK2 txq 0
pipeline P0 port out bsz 1 link LINK2 txq 1
pipeline P0 port out bsz 1 link LINK2 txq 2
pipeline P0 port out bsz 1 link LINK2 txq 3

pipeline P0 port out bsz 1 link LINK3 txq 0
pipeline P0 port out bsz 1 link LINK3 txq 1
pipeline P0 port out bsz 1 link LINK3 txq 2
pipeline P0 port out bsz 1 link LINK3 txq 3

;此处交换机被控制器指导工作，所以不用加载bin文件了
;pipeline P0 asm ./bin/output.bin

pipeline P0 protocol PROT0

;CE核
control 2 pipeline P0 io
control 4 pipeline P0 msg
;control 6 pipeline P0 update

;抓包核
module 6 pipeline P0 pcap

;此处port1 2 3 4一一对应我们绑定的四张网卡link0 1 2 3
thread 8 pipeline P0 enable port 1
thread 10 pipeline P0 enable port 2
thread 12 pipeline P0 enable port 3
thread 14 pipeline P0 enable port 4
